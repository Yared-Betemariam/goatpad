...
- Live Markdown highlighting using each theme's primary and secondary colors, a read-only rendered Markdown preview with consistent block spacing plus ordered and nested lists, and common formatting actions, available from the action bar, the Edit menu, or keyboard shortcuts; preview coloring can be toggled from View and is enabled by default
